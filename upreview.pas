...
uses
  ...
  {$IFDEF COMPILER4_UP} ImgList, {$ENDIF} Preview, ExtDlgs,_FichierCdn,unit3;
type
...
uses
  {$IFDEF COMPILER7_UP} XPMan, {$ENDIF} Math, Printers, Clipbrd, Types;
...
procedure TMainForm.GeneratePages(index:integer);
begin
  with PrintPreview do
  begin
    {$IFDEF COMPILER7_UP}
...
procedure TMainForm.DrawGrilleNotes;
var
  R: TRect;
  SavedBottom: Integer;
  ARow,ACol:integer;
...
function TMainForm.repos(R: TRect; x, y: real): TRect;
 var
  OneCM: TPoint;
begin
